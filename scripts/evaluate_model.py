...
# python scripts/evaluate_model.py NoBRS --exp-path=mymodel/MutiNet/000:180 --datasets=Berkeley
# python scripts/evaluate_model.py NoBRS --exp-path=mymodel/MutiNet/000:180 --datasets=DAVIS
# python scripts/evaluate_model.py NoBRS --exp-path=mymodel/MutiNet/001:129 --datasets=SBD
...
def parse_args():
    ...
    group_device.add_argument('--gpus', type=str, default='5',
                              help='ID of used GPU.')
    ...
    group_iou_thresh.add_argument('--iou-analysis', action='store_true', default=None,
...
def get_checkpoints_list_and_logs_path(args, cfg):
    ...
    if args.exp_path:
        ...
        if checkpoint_prefix:   # 指定了单一的 pth 文件
...
def get_prediction_vis_callback(logs_path, dataset_name, prob_thresh):
    ...
    def callback(image, gt_mask, pred_probs, feat, sample_id, click_indx, clicks_list):
        sample_path = save_path / f'{sample_id}_{click_indx}.jpg'
        # prob_map = draw_probmap(pred_probs)
        feat_map = draw_probmap(feat)
        image_with_mask = draw_with_blend_and_clicks(image, pred_probs > prob_thresh, clicks_list=clicks_list)
        cv2.imwrite(str(sample_path), np.concatenate((image_with_mask, feat_map), axis=1)[:, :, ::-1])
