from isegm.utils.exp_imports.default import *
from isegm.model.mymodel.CTB_ALF_LPP import MutiModel
from isegm.engine.TBAL_trainer import ISTrainer
MODEL_NAME = 'TBALIDNet'
from isegm.model.point_flow.model import Bottleneck

def main(cfg):
    model, model_cfg = init_model(cfg)
    train(model, cfg, model_cfg)


def init_model(cfg):
    model_cfg = edict()
    model_cfg.crop_size = (384, 384)
    model_cfg.num_max_points = 24

    model = MutiModel(backbone='resnet34', deeplab_ch=128, aspp_dropout=0.20, with_prev_mask=True, use_leaky_relu=True, use_rgb_conv=False, use_disks=True, norm_radius=5)
    model.to(cfg.device)
    model.apply(initializer.XavierGluon(rnd_type='gaussian', magnitude=2.0))
    model.img_encoder.load_pretrained_weights()
    return model, model_cfg

def load_weights(model, path_to_weights):
    current_state_dict = model.state_dict()
    new_state_dict = torch.load(path_to_weights, map_location='cpu')['state_dict']
    current_state_dict.update(new_state_dict)
    model.load_state_dict(current_state_dict)

def train(model, cfg, model_cfg):
    cfg.batch_size = 28 if cfg.batch_size < 1 else cfg.batch_size
    cfg.val_batch_size = cfg.batch_size
    crop_size = model_cfg.crop_size

    loss_cfg = edict()
    loss_cfg.instance_loss = NormalizedFocalLossSigmoid(alpha=0.5, gamma=2)
    loss_cfg.instance_loss_weight = 1.0
    loss_cfg.click_loss = click_NFL(alpha=0.5, gamma=2)
    loss_cfg.click_loss_weight = 0.2
    loss_cfg.prev_loss = prev_NFL(alpha=0.5, gamma=2)
    loss_cfg.prev_loss_weight = 0.1

    train_augmentator = Compose([
        UniformRandomResize(scale_range=(0.85, 1.10)),
        HorizontalFlip(),
        PadIfNeeded(min_height=crop_size[0], min_width=crop_size[1], border_mode=0),
        RandomCrop(*crop_size),
        RandomBrightnessContrast(brightness_limit=(-0.25, 0.25), contrast_limit=(-0.15, 0.4), p=0.75),
        RGBShift(r_shift_limit=10, g_shift_limit=10, b_shift_limit=10, p=0.75)
    ], p=1.0)   # Compose 可以把一些函数组织起来，上个函数的输出是下个函数的输入

    val_augmentator = Compose([
        PadIfNeeded(min_height=crop_size[0], min_width=crop_size[1], border_mode=0),
        RandomCrop(*crop_size)
    ], p=1.0)

    points_sampler = MultiPointSampler(model_cfg.num_max_points, prob_gamma=0.8,   # 点击策略
                                       merge_objects_prob=0.15,
                                       max_num_merged_objects=2)

    trainset = CocoDataset(
        cfg.COCO_PATH,
        split='train',
        augmentator=train_augmentator,
        min_object_area=1000,
        keep_background_prob=0.05,
        points_sampler=points_sampler,
        stuff_prob=0.30,
        epoch_len=30000
    )

    valset = CocoDataset(
        cfg.COCO_PATH,
        split='val',
        augmentator=val_augmentator,
        min_object_area=1000,
        keep_background_prob=0.05,
        points_sampler=points_sampler,
        epoch_len=2000
    )

    optimizer_params = {
        'lr': 5e-4, 'betas': (0.9, 0.999), 'eps': 1e-8
    }

    lr_scheduler = partial(torch.optim.lr_scheduler.MultiStepLR,    # 学习率的动态调整
                           milestones=[110, 120], gamma=0.1)
    trainer = ISTrainer(model, cfg, model_cfg, loss_cfg,
                        trainset, valset,
                        optimizer='adam',
                        optimizer_params=optimizer_params,
                        lr_scheduler=lr_scheduler,
                        checkpoint_interval=5,
                        image_dump_interval=1000,
                        metrics=[AdaptiveIoU()],   # 训练过程中的度量指标
                        max_interactive_points=model_cfg.num_max_points,
                        max_num_next_clicks=3)
    trainer.run(num_epochs=130)
