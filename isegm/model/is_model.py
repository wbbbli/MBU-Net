...
class ISModel(nn.Module):
    ...
    def forward(self, image, points, tak):
        if tak:
            outputs = self.fistclick(image, points)
        else:
            outputs = self.otherclick(image, points)
        return outputs

    def getmindis(self, points, embed):
        # print(points.shape)
        pointnum = points.shape[1]
        dis = []
        for i in range(points.shape[0]):
            pospoint = points[i, :pointnum//2, :2]
            negpoint = points[i, pointnum//2:, :2]
            pospoint = pospoint[pospoint[:, 0] > 0, :]
            negpoint = negpoint[negpoint[:, 0] > 0, :]
            posdis = self.getdis(pospoint, embed[i])
            negdis = self.getdis(negpoint, embed[i])
            dis.append(np.stack((posdis, negdis)))
        dis = torch.tensor(dis).to(embed.device)
        return dis

    def getdis(self, points, embed):
        embed = embed.clone().detach().cpu().numpy()
        points = points.cpu().numpy().astype(int)
        if points.shape[0]==0:
            return np.full((embed.shape[1],embed.shape[2]), 1e6, dtype=np.float32)
        # 获得点击对应像素的嵌入
        potemb = embed[:, points[:, 0], points[:, 1]]
        dis = []
        for i in range(potemb.shape[1]):
            emb = embed.reshape(embed.shape[0], embed.shape[1] * embed.shape[2]).swapaxes(0, 1)
            dis.append(np.linalg.norm(emb - potemb[:, i], axis=1))
        dis = np.array(dis)
        dis = np.min(dis, axis=0).reshape(embed.shape[1], embed.shape[2])
        return dis
