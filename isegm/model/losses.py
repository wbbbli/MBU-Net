...
class click_NFL(nn.Module):
    def __init__(self, axis=-1, alpha=0.25, gamma=2, max_mult=-1, eps=1e-12,
                 from_sigmoid=False, detach_delimeter=True,
                 batch_axis=0, weight=None, size_average=True,
                 ignore_label=-1):
        super(click_NFL, self).__init__()
        self._axis = axis
        self._alpha = alpha
        self._gamma = gamma
        self._ignore_label = ignore_label
        ...
        self._k_sum = 0
        self._m_max = 0
        self.distmap = DistMaps(norm_radius=20, spatial_scale=1.0,
                                cpu_mode=False, use_disks=True)

    def forward(self, pred, label, click):
        c_mask = self.distmap.get_coord_features(click, pred.shape[0], pred.shape[2], pred.shape[3])
        c_mask = (c_mask[:, 0, :, :] + c_mask[:, 1, :, :]).unsqueeze(dim=1)
        label = torch.where(c_mask > 0, label, torch.ones(label.shape, device=label.device) * -1)
...
class prev_NFL(nn.Module):
    def __init__(self, axis=-1, alpha=0.25, gamma=2, max_mult=-1, eps=1e-12,
                 from_sigmoid=False, detach_delimeter=True,
                 batch_axis=0, weight=None, size_average=True,
                 ignore_label=-1):
        super(prev_NFL, self).__init__()
        self._axis = axis
        self._alpha = alpha
        self._gamma = gamma
        self._ignore_label = ignore_label
    ...
    def forward(self, pred, label, prev):

        label = torch.where(prev > 0.49, label, torch.ones(label.shape, device=label.device) * -1)
