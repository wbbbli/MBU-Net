...
class BasePredictor(object):
    ...
    def set_input_image(self, image):
        ...
        self.ori_h = image_nd.shape[1]
        self.ori_w = image_nd.shape[2]
    ...
    def get_prediction(self, clicker, prev_mask=None):
        ...
        pred_logits, feat = self._get_prediction(image_nd, clicks_lists, is_image_changed)
        prediction = F.interpolate(pred_logits, mode='bilinear', align_corners=True,
                                   size=image_nd.size()[2:])
        feat = F.interpolate(feat, mode='bilinear', align_corners=True,
                             size=(self.ori_h, self.ori_w))
        feat = torch.mean(feat, dim=1).unsqueeze(dim=1)
        ...
        return prediction.cpu().numpy()[0, 0], feat.cpu().numpy()[0, 0]

    def _get_prediction(self, image_nd, clicks_lists, is_image_changed):
        points_nd = self.get_points_nd(clicks_lists)
        output = self.net(image_nd, points_nd, self.token)
        self.token = False
        return output['instances'], output['feat']
