...
class ISTrainer(object):
    def __init__(self, model, cfg, model_cfg, loss_cfg,
        ...
        self.train_data = DataLoader(
            ...
            sampler=get_sampler(trainset, shuffle=False, distributed=cfg.distributed),
    ...
    def training(self, epoch):   # 训练一轮
        ...
        for i, batch_data in enumerate(tbar):
            ...
            self.net.click_encoder.eval()
            self.net.decoder2.eval()
            self.net.decoder3.eval()
            self.net.seg.eval()
            self.net.click_seg.eval()
    ...
    def batch_forward(self, batch_data, validation=False):
        ...
        with torch.set_grad_enabled(not validation):
            ...
            with torch.no_grad():
                ...
                first_output = self.net(net_input, points, True)
                prev_output = torch.sigmoid(first_output['instances'])

                points = get_next_points(prev_output, orig_gt_mask, points, 1)
                if not validation:
                    self.net.train()
                #########################
                ...
                for click_indx in range(1, num_iters):
                    ...
                    prev_output = torch.sigmoid(self.net(net_input, points, False)['instances'])
            ...
            net_input = torch.cat((image, prev_output), dim=1) if self.net.with_prev_mask else image
            output = self.net(net_input, points, False)

            loss = 0.0
            loss = self.add_loss('prev_loss', loss, losses_logging, validation,
                                 lambda: (output['prev_aux'], batch_data['instances'], prev_output))
            loss = self.add_loss('click_loss', loss, losses_logging, validation,
                                 lambda: (output['click_aux'], batch_data['instances'], batch_data['points']))
            loss = self.add_loss('instance_loss', loss, losses_logging, validation,
                                 lambda: (output['instances'],
