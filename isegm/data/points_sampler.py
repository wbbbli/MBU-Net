...
class MultiPointSampler(BasePointSampler):
    def __init__(self, max_num_points, prob_gamma=0.7, expand_ratio=0.1,
                 ...
                 sfc_inner_k=1.7, sfc_full_inner_prob=0.0, pos_edg_dis=10, point_dis = 5):
        super().__init__()
    ...
    def _sample_mask(self, sample: DSample):
        ...
        obj_id = random.choice(root_obj_ids)
    ...
    def _multi_mask_sample_points(self, selected_masks, is_negative, with_first_click=False):
        ...
        if is_negative:
            each_obj_points = [
                self._sample_points_neg(mask)
                for i, mask in enumerate(selected_masks)
            ]
        else:
            each_obj_points = [
                self._sample_points_pos(mask, edg_dis=self.pos_edg_dis)
                for i, mask in enumerate(selected_masks)
            ]
    ...
    def _sample_points_neg(self, mask):
        ...
        for j in range(num_points):
            ...
            point_indices = indices[point_indices_indx][0]  # 获得对应策略标签内的索引
    ...
    def _sample_points_pos(self, mask, edg_dis=10):
        ...
        for j in range(num_points):
            first_click = j == 0
            point_indices = np.argwhere(mask)
            num_indices = len(point_indices)
            if num_indices > 0:
                point_indx = 0 if first_click else 100
                a = np.random.randint(0, num_indices)
                click = point_indices[a].tolist() + [
                    point_indx]
                if len(click)<3:
                    continue
                points.append(click)
                point_dis = int(np.ceil(np.sqrt(num_indices) / 2)) if num_indices < (self.point_dis*2)**2 else self.point_dis
                mask = get_point_mask(mask, click, pon_dis=point_dis)
...
def get_point_mask(mask, pointinds, pon_dis=5):
    point_mask = np.ones(mask.shape)
    point_mask[pointinds[0], pointinds[1]] = 0
    padded_mask = np.pad(point_mask, ((1, 1), (1, 1)), 'constant')
    dt = cv2.distanceTransform(padded_mask.astype(np.uint8), cv2.DIST_L2, 0)[1:-1, 1:-1]
    dt = dt >pon_dis
    remask = np.logical_and(mask, dt)
    return remask